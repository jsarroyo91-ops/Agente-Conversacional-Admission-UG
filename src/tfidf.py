...
def extraer_utterances(datos_intents: Dict[str, Any]) -> List[Dict[str, str]]:
    """
    Extrae las utterances del archivo intents.json.

    Cada utterance queda asociada a su intención o tag.

    Args:
        datos_intents (Dict[str, Any]): datos cargados desde intents.json.

    Returns:
        List[Dict[str, str]]: lista de utterances con su intención.
    """
    registros = []
    ...
    for intent in datos_intents.get("intents", []):
        ...
        for utterance in utterances:
            texto_preprocesado = preprocesar_texto(utterance)
